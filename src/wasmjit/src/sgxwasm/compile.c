...
dump_control_stack(struct Control* control_stack)
{
  ...
  for (idx = control_stack->size - 1; idx >= 0; idx--) {
    ...
    printf("in: %u, out: %u, start_reached: %u, end_reached: %u,",
           c->in_arity,
           c->out_arity,
           c->start_reached,
           c->end_reached);
    printf(" label: %d, stack height: %zu, else_label: %d",
           c->label.pos,
           stack_height(&c->label_state),
           c->else_state.label.pos);
    printf("\n");
  }
...
#if MEMORY_TRACE
static void
record_memory_access(struct MemoryTracer* mem_tracer,
                     size_t index,
...
__attribute__((unused)) static void
uncond_jmp(struct CompilerContext* ctx,
           label_t* label,
           uint32_t depth,
           distance_t distance)
{
...
#endif
  emit_jmp_label(output(ctx), label, distance);
#if __PASS__
...
__attribute__((unused)) static void
cond_jmp(struct CompilerContext* ctx,
         condition_t cond,
         label_t* label,
...
__attribute__((unused)) static void
br_table_cond_jmp(struct CompilerContext* ctx,
                  condition_t cond,
                  label_t* label,
                  sgxwasm_valtype_t type,
                  sgxwasm_register_t lhs,
...
br_table_uncond_jmp(struct CompilerContext* ctx, label_t* label, uint32_t depth)
{
  ...
  passes_machine_inst_start(ctx, &minstr);
#endif
  emit_jmp_label(output(ctx), label, Far);
#if __PASS__
  passes_machine_inst_end(ctx, &minstr);
#endif
}
...
__attribute__((unused)) static void
br_table_bind(struct CompilerContext* ctx,
              label_t* label,
              uint8_t type,
              uint32_t min,
              uint32_t max,
...
// Wrapper of Spill.
__attribute__((unused)) static void
spill_to_stack(struct CompilerContext* ctx,
               uint32_t index,
...
// Wrapper of Fill.
__attribute__((unused)) static void
fill_from_stack(struct CompilerContext* ctx,
                sgxwasm_register_t reg,
                uint32_t index,
                sgxwasm_valtype_t type)
{
  ...
  passes_machine_inst_start(ctx, &minstr);
#endif
  Fill(output(ctx), reg, index, type);
#if __PASS__
  passes_machine_inst_end(ctx, &minstr);
#endif
}
...
__attribute__((unused)) static void
load_const_to_reg(struct CompilerContext* ctx,
                  sgxwasm_register_t reg,
                  int64_t val,
                  sgxwasm_valtype_t type)
{
  ...
  passes_machine_inst_start(ctx, &minstr);
#endif
  LoadConstant(output(ctx), reg, val, type);
#if __PASS__
  passes_machine_inst_end(ctx, &minstr);
#endif
}
...
// Wrapper of LoadConstant.
__attribute__((unused)) static void
call_function(struct CompilerContext* ctx,
...
split_state(struct CacheState* dst, struct CacheState* src)
{
  ...
  memcpy(dst->stack_state->data,
         src->stack_state->data,
  ...
  memcpy(dst->register_use_count,
         src->register_use_count,
...
static void
make_slot(struct StackSlot* slot,
          sgxwasm_valtype_t type,
          location_t loc,
          sgxwasm_register_t reg,
          int32_t i32_const)
...
static void
make_slot_register(struct StackSlot* slot,
                   sgxwasm_valtype_t type,
                   sgxwasm_register_t reg)
{
...
static void
make_slot_const(struct StackSlot* slot,
                sgxwasm_valtype_t type,
                int32_t i32_const)
{
...
static int
push_stack(struct CacheState* cache_state,
           sgxwasm_valtype_t type,
           location_t loc,
           sgxwasm_register_t reg,
           int32_t val)
...
static int
push_register(struct CompilerContext* ctx,
              sgxwasm_valtype_t type,
              sgxwasm_register_t reg)
{
...
static label_t*
add_out_of_line_trap(struct OutOfLineCodeList* ool_list,
                     size_t position,
                     uint32_t stub,
                     size_t pc)
{
...
static void
add_param_at(struct LocationSignature* sig,
             size_t index,
...
__attribute__((unused)) static void
load_const(struct StackTransferRecipe* stack_transfer,
           sgxwasm_register_t dst,
           uint32_t value,
           sgxwasm_valtype_t type)
{
...
// Forward declaration.
static void
clear_executed_move(struct CompilerContext*,
                    struct StackTransferRecipe*,
                    sgxwasm_register_t);

__attribute__((unused)) static void
execute_move(struct CompilerContext* ctx,
             ...
             sgxwasm_register_t dst)
{
  ...
  Move(output(ctx), dst, move->src, move->type);
              ...
              struct StackTransferRecipe* transfers)
{
  ...
  while (!is_empty(transfers->move_dst_regs)) {
    ...
    if (!mem_tracer_grow(mem_tracer(ctx)))
      assert(0);
    record_memory_access(mem_tracer(ctx),
                         c->cont,
                         c->type,
...
__attribute__((unused)) static void
move_stack_value(struct CompilerContext* ctx,
                 int dst_idx,
                 int src_idx,
                 sgxwasm_valtype_t type)
{
...
transfer_stack_slot(struct CompilerContext* ctx,
                    struct StackTransferRecipe* transfers,
                    struct StackState* dst_stack,
                    struct StackState* src_stack,
                    int dst_idx,
                    int src_idx)
{
...
{
  if (ctx->control_stack.data) {
    free(ctx->control_stack.data);
  }
  if (ctx->ool_list.data) {
    free(ctx->ool_list.data);
  }
}
...
spill(struct CompilerContext* ctx, uint32_t index)
{
  ...
  switch (slot->loc) {
    ...
    case LOC_CONST: {
      spill_to_stack(
        ctx, index, REG_UNKNOWN, (int64_t)slot->i32_const, slot->type);
#if MEMORY_TRACE
      struct ControlBlock* c =
        &control(ctx)->data[control_depth(control(ctx)) - 1];
      int offset = index * StackSlotSize * -1 - StackOffset;
...
static sgxwasm_register_t
spill_one_register(struct CompilerContext* ctx,
                   ...
                   reglist_t pinned)
{
  ...
  sgxwasm_register_t spill_reg =
    get_next_spill_reg(*(cache_state(ctx)->used_registers),
                       cache_state(ctx)->last_spilled_regs,
                       candidates,
                       pinned);
...
static void
push_stack_slot(struct CompilerContext* ctx,
  ...
  switch (slot->loc) {
    case LOC_STACK: {
      if (type == VALTYPE_I32) {
        ...
        emit_pushq_r(output(ctx), ScratchGP);
      } else {
        // XXX: Check F32
        emit_pushq_m(output(ctx), &src);
      }
      break;
    }
    case LOC_REGISTER: {
      Push(output(ctx), slot->reg, slot->type);
      break;
    }
    case LOC_CONST: {
      emit_pushq_i(output(ctx), slot->i32_const);
      break;
    }
    default:
      assert(0);
...
static sgxwasm_register_t
get_unused_register(struct CompilerContext* ctx,
                    reglist_t candidates,
                    reglist_t pinned)
{
  sgxwasm_register_t reg;
  if (has_unused_register(
        candidates, *(cache_state(ctx)->used_registers), pinned)) {
    reg =
...
pop_to_register(struct CompilerContext* ctx, reglist_t pinned)
{
  ...
  struct StackSlot slot;
  memcpy(&slot,
         &cache_state(ctx)->stack_state->data[height - 1],
...
static DEFINE_VECTOR_GROW(reusemap, struct RegisterReuseMap);
...
static void
add_pair(struct RegisterReuseMap* map,
         sgxwasm_register_t src,
         sgxwasm_register_t dst)
{
...
__attribute__((unused)) static void
init_merge_region(struct CacheState* dst_state,
                  uint32_t dst_begin,
                  struct CacheState* src_state,
                  uint32_t src_begin,
                  uint32_t count,
                  uint8_t keep_stack_slots,
                  uint8_t allow_constants,
                  uint8_t reuse_registers,
                  reglist_t used_reg)
{
...
__attribute__((unused)) static void
init_merge(struct CacheState* dst_state,
           struct CacheState* src_state,
           uint32_t num_locals,
           uint32_t arity)
{
  ...
  init_merge_region(dst_state,
                    dst_begin + dst_state->stack_base,
                    src_state,
                    src_begin + dst_state->stack_base + discarded,
                    arity,
                    keep_merge_stack_slots,
                    0 /* False */,
                    0 /* False */,
                    used_regs);
  ...
  init_merge_region(dst_state,
                    dst_begin + num_locals,
                    src_state,
                    src_begin + num_locals,
                    dst_state->stack_base - num_locals,
                    1 /* keep_stack_slots = True */,
                    1 /* allow_constants = True */,
                    1 /* reuse_registers = True */,
                    used_regs);
}

__attribute__((unused)) static void
merge_full_stack_with(struct CompilerContext* ctx,
                      struct CacheState* dst_state,
                      struct CacheState* src_state)
{
#if SGXWASM_DEBUG_COMPILE
  printf(
    "[merge_full_stack_with] dst stack height: %lu, src stack height: %lu\n",
    stack_height(dst_state),
    stack_height(src_state));
#endif
  ...
  for (i = 0; i < (int)stack_height(src_state); i++) {
    transfer_stack_slot(
      ctx, &transfers, dst_state->stack_state, src_state->stack_state, i, i);
  }
  execute_transfers(ctx, &transfers);
}

__attribute__((unused)) static void
merge_stack_with(struct CompilerContext* ctx,
                 struct CacheState* dst_state,
                 struct CacheState* src_state,
                 uint32_t arity)
{
  ...
  for (i = 0; i < (int)arity; i++) {
    transfer_stack_slot(ctx,
                        &transfers,
                        dst_state->stack_state,
                        src_state->stack_state,
                        dst_stack_base + i,
                        src_stack_base + i);
  }
  execute_transfers(ctx, &transfers);
...
static void
load_from_memory(struct CompilerContext* ctx,
                 sgxwasm_register_t dst,
                 size_t mem_type,
                 size_t index)
{
  ...
  emit_movq_ri(output(ctx), dst, 0xffffffffffffffff);
...
push_control(struct Control* control_stack,
             control_type_t type,
             uint32_t in_arity,
             uint32_t out_arity)
{
...
return_impl(struct CompilerContext* ctx)
{
  ...
  // Leave frame.
  Move(output(ctx), GP_RSP, GP_RBP, VALTYPE_I64);
  emit_popq_r(output(ctx), GP_RBP);
...
static void
br_impl(struct CompilerContext* ctx,
        struct ControlBlock* target,
        uint32_t depth)
{
  ...
  if (!target_reached) {
    init_merge(
      &target->label_state, cache_state(ctx), num_locals(ctx), target_arity);
  }
...
__attribute__((unused)) static void
generate_br_case(struct CompilerContext* ctx,
                 uint32_t br_depth,
                 label_t* br_targets)
{
...
__attribute__((unused)) static void
generate_br_table(struct CompilerContext* ctx,
                  ...
                  label_t* br_targets)
{
  ...
  br_table_cond_jmp(
    ctx, COND_GE_U, &upper_half, VALTYPE_I32, value, tmp, min, max);

  // Emit br table for lower half.
  generate_br_table(
    ctx, tmp, value, min, split, table, iterator, table_count, br_targets);
  ...
  generate_br_table(
    ctx, tmp, value, split, max, table, iterator, table_count, br_targets);
}
...
static sgxwasm_register_t
get_global_addr(struct CompilerContext* ctx,
                size_t global_index,
                reglist_t* pinned,
                size_t* offset)
{
...
__attribute__((unused)) static void
prepare_call(struct CompilerContext* ctx,
             const struct FuncType* fun_type,
             sgxwasm_register_t* target)
{
  ...
  if (num_params > 6 && (num_params % 2 == 1)) {
    // Stack padding.
    emit_pushq_r(output(ctx), GP_RAX);
  }
...
__attribute__((unused)) static void
call_direct(struct CompilerContext* ctx,
...
__attribute__((unused)) static void
EmitTypeConversion(struct CompilerContext* ctx,
                   ...
                   size_t trap_position)
{
  ...
  label_t* trap =
    can_trap
      ? add_out_of_line_trap(
          out_of_line_code(ctx), trap_position, TrapFloatUnrepresentable, 0)
      : NULL;
...
__attribute__((unused)) static void
EmitShiftOp(struct CompilerContext* ctx,
            sgxwasm_register_t dst,
            sgxwasm_register_t src,
            sgxwasm_register_t amount,
            sgxwasm_valtype_t type,
            uint8_t opcode)
{
  // If dst is rcx, compute into the scratch register first, then move to rcx.
  if (dst == GP_RCX) {
    Move(output(ctx), ScratchGP, src, type);
    if (amount != GP_RCX) {
      Move(output(ctx), GP_RCX, amount, type);
    }
    Shift(output(ctx), ScratchGP, opcode);
    Move(output(ctx), GP_RCX, ScratchGP, type);
    return;
  }
  ...
  if (dst != src) {
    Move(output(ctx), dst, src, type);
  }
  // Do the actual shift.
  Shift(output(ctx), dst, opcode);
...
__attribute__((unused)) static void
EmitUnOp(struct CompilerContext* ctx,
         sgxwasm_valtype_t src_type,
         sgxwasm_valtype_t result_type,
         uint8_t opcode)
{
  sgxwasm_register_class_t src_rc = reg_class_for(src_type);
  ...
  if (src_rc == result_rc) {
    ...
    dst = get_unused_register_with_class(
      ctx, result_rc, EmptyRegList, EmptyRegList);
  }

  switch (opcode) {
    case OPCODE_I32_EQZ: {
      assert(src_type == VALTYPE_I32 && result_type == VALTYPE_I32);
      emit_test_rr(output(ctx), src, src, VALTYPE_I32);
      emit_setcc(output(ctx), COND_EQ, dst);
      emit_movzxb_rr(output(ctx), dst, dst);
      break;
    }
    case OPCODE_I32_CLZ: {
      assert(src_type == VALTYPE_I32 && result_type == VALTYPE_I32);
      ...
      emit_movl_ri(output(ctx), dst, 32);
      emit_jmp_label(output(ctx), &continuation, Near);

      bind_label(output(ctx), &nonzero_input, pc_offset(output(ctx)));
      // Get most significant bit set (MSBS).
      emit_bsr_rr(output(ctx), dst, src, VALTYPE_I32);
      // CLZ = 31 - MSBS = MSBS ^ 31.
      emit_xor_ri(output(ctx), dst, 31, VALTYPE_I32);
    ...
    case OPCODE_I32_CTZ: {
      ...
      emit_bsf_rr(output(ctx), dst, src, VALTYPE_I32);
    ...
    case OPCODE_F32_ABS: {
      ...
      if (dst == src) {
        ...
        emit_andps_rr(output(ctx), dst, ScratchFP);
      } else {
        emit_movl_ri(output(ctx), ScratchGP, (int32_t)(SIGN_BIT_32 - 1));
        emit_sse_movd_rr(output(ctx), dst, ScratchGP);
        emit_andps_rr(output(ctx), dst, src);
      }
      break;
    }
    case OPCODE_F32_NEG: {
      ...
      if (dst == src) {
        ...
        emit_sse_movd_rr(output(ctx), ScratchFP, ScratchGP);
        emit_xorps_rr(output(ctx), dst, ScratchFP);
      } else {
        emit_movl_ri(output(ctx), ScratchGP, (int32_t)SIGN_BIT_32);
        emit_sse_movd_rr(output(ctx), dst, ScratchGP);
        emit_xorps_rr(output(ctx), dst, src);
      }
      break;
    }
    case OPCODE_F32_CEIL: {
      assert(src_type == VALTYPE_F32 && result_type == VALTYPE_F32);
      emit_roundss_rr(output(ctx), dst, src, RoundUp);
      break;
    }
    case OPCODE_F32_FLOOR: {
      assert(src_type == VALTYPE_F32 && result_type == VALTYPE_F32);
      emit_roundss_rr(output(ctx), dst, src, RoundDown);
      break;
    }
    ...
    case OPCODE_F32_SQRT: {
      assert(src_type == VALTYPE_F32 && result_type == VALTYPE_F32);
      emit_sqrtss_rr(output(ctx), dst, src);
      break;
    }
    case OPCODE_F64_ABS: {
      ...
      if (dst == src) {
        ...
        emit_andpd_rr(output(ctx), dst, ScratchFP);
      } else {
        emit_movq_ri(output(ctx), ScratchGP, (int64_t)(SIGN_BIT_64 - 1));
        emit_sse_movq_rr(output(ctx), dst, ScratchGP);
        emit_andpd_rr(output(ctx), dst, src);
      }
      break;
    }
    case OPCODE_F64_NEG: {
      ...
      if (dst == src) {
        ...
        emit_sse_movq_rr(output(ctx), ScratchFP, ScratchGP);
        emit_xorpd_rr(output(ctx), dst, ScratchFP);
      } else {
        emit_movq_ri(output(ctx), ScratchGP, (int64_t)SIGN_BIT_64);
        emit_sse_movq_rr(output(ctx), dst, ScratchGP);
        emit_xorpd_rr(output(ctx), dst, src);
      }
      break;
    }
    case OPCODE_F64_CEIL: {
      assert(src_type == VALTYPE_F64 && result_type == VALTYPE_F64);
      emit_roundsd_rr(output(ctx), dst, src, RoundUp);
      break;
    }
    case OPCODE_F64_FLOOR: {
      assert(src_type == VALTYPE_F64 && result_type == VALTYPE_F64);
      emit_roundsd_rr(output(ctx), dst, src, RoundDown);
      break;
    }
    ...
    case OPCODE_F64_SQRT: {
      assert(src_type == VALTYPE_F64 && result_type == VALTYPE_F64);
      emit_sqrtsd_rr(output(ctx), dst, src);
      break;
    }
    default:
      break;
...
__attribute__((unused)) static void
EmitBinOp(struct CompilerContext* ctx,
          sgxwasm_valtype_t src_type,
          sgxwasm_valtype_t result_type,
          uint8_t opcode)
{
  ...
  switch (opcode) {
    ...
    case OPCODE_I32_ADD: {
      ...
      if (lhs != dst) {
        ...
        emit_lea_rm(output(ctx), dst, &op, VALTYPE_I32);
      } else {
        emit_add_rr(output(ctx), dst, rhs, VALTYPE_I32);
      }
      break;
    }
    case OPCODE_I32_SUB: {
      assert(src_type == VALTYPE_I32 && result_type == VALTYPE_I32);
      if (dst == rhs) {
        emit_neq_r(output(ctx), dst, VALTYPE_I32);
        emit_add_rr(output(ctx), dst, lhs, VALTYPE_I32);
      } else {
        if (dst != lhs) {
          emit_mov_rr(output(ctx), dst, lhs, VALTYPE_I32);
        }
        emit_sub_rr(output(ctx), dst, rhs, VALTYPE_I32);
      }
      break;
    }
    case OPCODE_I32_MUL: {
      assert(src_type == VALTYPE_I32 && result_type == VALTYPE_I32);
      if (dst == rhs) {
        emit_imul_rr(output(ctx), dst, lhs, VALTYPE_I32);
      } else {
        if (dst != lhs) {
          emit_mov_rr(output(ctx), dst, lhs, VALTYPE_I32);
        }
        emit_imul_rr(output(ctx), dst, rhs, VALTYPE_I32);
      }
      break;
    }
    case OPCODE_I32_AND: {
      assert(src_type == VALTYPE_I32 && result_type == VALTYPE_I32);
      if (dst == rhs) {
        emit_and_rr(output(ctx), dst, lhs, VALTYPE_I32);
      } else {
        if (dst != lhs) {
          emit_mov_rr(output(ctx), dst, lhs, VALTYPE_I32);
        }
        emit_and_rr(output(ctx), dst, rhs, VALTYPE_I32);
      }
      break;
    }
    case OPCODE_I32_OR: {
      assert(src_type == VALTYPE_I32 && result_type == VALTYPE_I32);
      if (dst == rhs) {
        emit_or_rr(output(ctx), dst, lhs, VALTYPE_I32);
      } else {
        if (dst != lhs) {
          emit_mov_rr(output(ctx), dst, lhs, VALTYPE_I32);
        }
        emit_or_rr(output(ctx), dst, rhs, VALTYPE_I32);
      }
      break;
    }
    case OPCODE_I32_XOR: {
      assert(src_type == VALTYPE_I32 && result_type == VALTYPE_I32);
      if (dst == rhs) {
        emit_xor_rr(output(ctx), dst, lhs, VALTYPE_I32);
      } else {
        if (dst != lhs) {
          emit_mov_rr(output(ctx), dst, lhs, VALTYPE_I32);
        }
        emit_xor_rr(output(ctx), dst, rhs, VALTYPE_I32);
      }
      break;
    }
    ...
    case OPCODE_I64_ADD: {
      ...
      if (lhs != dst) {
        struct Operand op;
        build_operand(&op, lhs, rhs, SCALE_1, 0);
        emit_lea_rm(output(ctx), dst, &op, VALTYPE_I64);
      } else {
        emit_add_rr(output(ctx), dst, rhs, VALTYPE_I64);
      }
      break;
    }
    case OPCODE_I64_SUB: {
      assert(src_type == VALTYPE_I64 && result_type == VALTYPE_I64);
      if (dst == rhs) {
        emit_neq_r(output(ctx), dst, VALTYPE_I64);
        emit_add_rr(output(ctx), dst, lhs, VALTYPE_I64);
      } else {
        if (dst != lhs) {
          emit_mov_rr(output(ctx), dst, lhs, VALTYPE_I64);
        }
        emit_sub_rr(output(ctx), dst, rhs, VALTYPE_I64);
      }
      break;
    }
    case OPCODE_I64_MUL: {
      assert(src_type == VALTYPE_I64 && result_type == VALTYPE_I64);
      if (dst == rhs) {
        emit_imul_rr(output(ctx), dst, lhs, VALTYPE_I64);
      } else {
        if (dst != lhs) {
          emit_mov_rr(output(ctx), dst, lhs, VALTYPE_I64);
        }
        emit_imul_rr(output(ctx), dst, rhs, VALTYPE_I64);
      }
      break;
    }
    case OPCODE_I64_AND: {
      assert(src_type == VALTYPE_I64 && result_type == VALTYPE_I64);
      if (dst == rhs) {
        emit_and_rr(output(ctx), dst, lhs, VALTYPE_I64);
      } else {
        if (dst != lhs) {
          emit_mov_rr(output(ctx), dst, lhs, VALTYPE_I64);
        }
        emit_and_rr(output(ctx), dst, rhs, VALTYPE_I64);
      }
      break;
    }
    case OPCODE_I64_OR: {
      assert(src_type == VALTYPE_I64 && result_type == VALTYPE_I64);
      if (dst == rhs) {
        emit_or_rr(output(ctx), dst, lhs, VALTYPE_I64);
      } else {
        if (dst != lhs) {
          emit_mov_rr(output(ctx), dst, lhs, VALTYPE_I64);
        }
        emit_or_rr(output(ctx), dst, rhs, VALTYPE_I64);
      }
      break;
    }
    case OPCODE_I64_XOR: {
      assert(src_type == VALTYPE_I64 && result_type == VALTYPE_I64);
      if (dst == rhs) {
        emit_xor_rr(output(ctx), dst, lhs, VALTYPE_I64);
      } else {
        if (dst != lhs) {
          emit_mov_rr(output(ctx), dst, lhs, VALTYPE_I64);
        }
        emit_xor_rr(output(ctx), dst, rhs, VALTYPE_I64);
      }
      break;
    }
    ...
    case OPCODE_F32_ADD: {
      assert(src_type == VALTYPE_F32 && result_type == VALTYPE_F32);
      if (dst == lhs) {
        emit_addss_rr(output(ctx), dst, lhs);
      } else {
        if (dst != lhs) {
          emit_movss_rr(output(ctx), dst, lhs);
        }
        emit_addss_rr(output(ctx), dst, rhs);
      }
      break;
    }
    case OPCODE_F32_SUB: {
      assert(src_type == VALTYPE_F32 && result_type == VALTYPE_F32);
      if (dst == lhs) {
        emit_subss_rr(output(ctx), dst, lhs);
      } else {
        if (dst != lhs) {
          emit_movss_rr(output(ctx), dst, lhs);
        }
        emit_subss_rr(output(ctx), dst, rhs);
      }
      break;
    }
    case OPCODE_F32_MUL: {
      assert(src_type == VALTYPE_F32 && result_type == VALTYPE_F32);
      if (dst == lhs) {
        emit_mulss_rr(output(ctx), dst, lhs);
      } else {
        if (dst != lhs) {
          emit_movss_rr(output(ctx), dst, lhs);
        }
        emit_mulss_rr(output(ctx), dst, rhs);
      }
      break;
    }
    case OPCODE_F32_DIV: {
      assert(src_type == VALTYPE_F32 && result_type == VALTYPE_F32);
      if (dst == lhs) {
        emit_divss_rr(output(ctx), dst, lhs);
      } else {
        if (dst != lhs) {
          emit_movss_rr(output(ctx), dst, lhs);
        }
        emit_divss_rr(output(ctx), dst, rhs);
      }
      break;
    }
    case OPCODE_F32_MIN: {
      assert(src_type == VALTYPE_F32 && result_type == VALTYPE_F32);
      emit_float_min_or_max(output(ctx), dst, lhs, rhs, FLOAT_MIN, VALTYPE_F32);
      break;
    }
    case OPCODE_F32_MAX: {
      assert(src_type == VALTYPE_F32 && result_type == VALTYPE_F32);
      emit_float_min_or_max(output(ctx), dst, lhs, rhs, FLOAT_MAX, VALTYPE_F32);
      break;
    }
    case OPCODE_F32_COPYSIGN: {
      assert(src_type == VALTYPE_F32 && result_type == VALTYPE_F32);
      const int32_t sign_bit = 1 << 31;
      emit_sse_movd_rr(output(ctx), ScratchGP, lhs);
      emit_and_ri(output(ctx), ScratchGP, ~sign_bit, VALTYPE_I32);
      emit_sse_movd_rr(output(ctx), ScratchGP2, rhs);
    ...
    case OPCODE_F64_ADD: {
      assert(src_type == VALTYPE_F64 && result_type == VALTYPE_F64);
      if (dst == lhs) {
        emit_addsd_rr(output(ctx), dst, lhs);
      } else {
        if (dst != lhs) {
          emit_movsd_rr(output(ctx), dst, lhs);
        }
        emit_addsd_rr(output(ctx), dst, rhs);
      }
      break;
    }
    case OPCODE_F64_SUB: {
      assert(src_type == VALTYPE_F64 && result_type == VALTYPE_F64);
      if (dst == lhs) {
        emit_subsd_rr(output(ctx), dst, lhs);
      } else {
        if (dst != lhs) {
          emit_movsd_rr(output(ctx), dst, lhs);
        }
        emit_subsd_rr(output(ctx), dst, rhs);
      }
      break;
    }
    case OPCODE_F64_MUL: {
      assert(src_type == VALTYPE_F64 && result_type == VALTYPE_F64);
      if (dst == lhs) {
        emit_mulsd_rr(output(ctx), dst, lhs);
      } else {
        if (dst != lhs) {
          emit_movsd_rr(output(ctx), dst, lhs);
        }
        emit_mulsd_rr(output(ctx), dst, rhs);
      }
      break;
    }
    case OPCODE_F64_DIV: {
      assert(src_type == VALTYPE_F64 && result_type == VALTYPE_F64);
      if (dst == lhs) {
        emit_divsd_rr(output(ctx), dst, lhs);
      } else {
        if (dst != lhs) {
          emit_movsd_rr(output(ctx), dst, lhs);
        }
        emit_divsd_rr(output(ctx), dst, rhs);
      }
      break;
    }
    case OPCODE_F64_MIN: {
      assert(src_type == VALTYPE_F64 && result_type == VALTYPE_F64);
      emit_float_min_or_max(output(ctx), dst, lhs, rhs, FLOAT_MIN, VALTYPE_F64);
      break;
    }
    case OPCODE_F64_MAX: {
      assert(src_type == VALTYPE_F64 && result_type == VALTYPE_F64);
      emit_float_min_or_max(output(ctx), dst, lhs, rhs, FLOAT_MAX, VALTYPE_F64);
      break;
    }
    case OPCODE_F64_COPYSIGN: {
      assert(src_type == VALTYPE_F64 && result_type == VALTYPE_F64);
      emit_sse_movq_rr(output(ctx), ScratchGP2, rhs);
      emit_shr_ri(output(ctx), ScratchGP2, 63, VALTYPE_I64);
      emit_shl_ri(output(ctx), ScratchGP2, 63, VALTYPE_I64);
      emit_sse_movq_rr(output(ctx), ScratchGP, lhs);
      emit_btr_ri(output(ctx), ScratchGP, 63);
...
// TODO: Support call_indirect later.
__attribute__((unused)) static void
call_indirect(struct CompilerContext* ctx,
              const struct TypeSection* type_table,
              uint32_t type_index)
{
  const struct FuncType* fun_type = &type_table->types[type_index];
  // Pop the index.
  ...
  if (is_used(*(cache_state(ctx)->used_registers),
    ...
    sgxwasm_register_t new_index =
      get_unused_register_with_class(ctx, GP_REG, EmptyRegList, EmptyRegList);
    Move(output(ctx), new_index, index, VALTYPE_I32);
    index = new_index;
  }
  ...
  emit_call_r(output(ctx), scratch);
...
// of the function.
__attribute__((unused)) static int
bounds_check_mem(struct CompilerContext* ctx,
                 ...
                 mem_oob_check_t check_type)
{
  ...
  size_t position = pc_offset(output(ctx));
  label_t* trap_label = add_out_of_line_trap(
    out_of_line_code(ctx), position, TrapMemOutOfBounds, 0);

  if (static_oob) {
    emit_jmp_label(output(ctx), trap_label, Far);
  ...
  while (control_depth(control(ctx)) != 0) {
    ...
    switch (opcode) {
      ...
      case OPCODE_IF: {
        ...
        cond_jmp(
          ctx, COND_EQ, &c->else_state.label, VALTYPE_I32, reg, REG_UNKNOWN, 0);
        c->label_state.stack_base = stack_height(cache_state(ctx));
      ...
      case OPCODE_ELSE: {
        ...
        if (reachable(c)) {
          if (!c->end_reached) {
            init_merge(
              &c->label_state, cache_state(ctx), num_locals(ctx), c->out_arity);
          }
      ...
      case OPCODE_SELECT: {
        ...
        emit_cond_jump_rr(output(ctx),
                          ...
                          REG_UNKNOWN);
        if (dst != true_value) {
          Move(output(ctx), dst, true_value, type);
        }
        emit_jmp_label(output(ctx), &cont, Far);

        bind_label(output(ctx), &case_false, pc_offset(output(ctx)));
        if (dst != false_value) {
          Move(output(ctx), dst, false_value, type);
        }
      ...
      case OPCODE_BR_TABLE: {
        ...
        if (reachable(c)) {
          ...
          if (table_count > 0) {
            ...
            br_table_cond_jmp(
              ctx, COND_GE_U, &case_default, VALTYPE_I32, value, tmp, 0, 0);

            generate_br_table(ctx,
                              tmp,
                              value,
                              0 /* min */,
                              table_count /* max */,
      ...
      case OPCODE_GET_GLOBAL: {
        ...
        value = get_unused_register_with_class(
          ctx, reg_class_for(type), EmptyRegList, pinned);
        Load(output(ctx), value, addr, REG_UNKNOWN, offset, load_type, NULL, 1);
      ...
      case OPCODE_SET_GLOBAL: {
        ...
        Store(output(ctx), addr, REG_UNKNOWN, offset, reg, store_type, NULL, 1);
        break;
      }
      ...
      case OPCODE_F64_CONST: {
        float val = instr->data.f64_const.value;
        sgxwasm_register_t reg = get_unused_register_with_class(
      ...
      case OPCODE_I32_TRUNC_S_F32: {
        EmitTypeConversion(
          ctx, VALTYPE_I32, VALTYPE_F32, opcode, 1, pc_offset(output(ctx)));
        break;
      }
      ...
      case OPCODE_I32_TRUNC_S_F64: {
        EmitTypeConversion(
          ctx, VALTYPE_I32, VALTYPE_F64, opcode, 1, pc_offset(output(ctx)));
        break;
      }
      ...
      case OPCODE_I64_TRUNC_S_F32: {
        EmitTypeConversion(
          ctx, VALTYPE_I64, VALTYPE_F32, opcode, 1, pc_offset(output(ctx)));
        break;
      }
      ...
      case OPCODE_I64_TRUNC_S_F64: {
        EmitTypeConversion(
          ctx, VALTYPE_I64, VALTYPE_F64, opcode, 1, pc_offset(output(ctx)));
        break;
      }
...
process_parameters(struct CompilerContext* ctx,
                   struct LocationSignature* fun_sig,
                   sgxwasm_valtype_t type,
                   size_t index)
{
  sgxwasm_register_class_t rc = reg_class_for(type);
...
{
  uint64_t offset;
  emit_pushq_r(output(ctx), GP_RBP);
  Move(output(ctx), GP_RBP, GP_RSP, VALTYPE_I64);
  emit_subq_sp_32(output(ctx), 0);
                         ...
                         unsigned flags)
{
...
#if __OPT_INIT_STACK__
            push_stack(
              cache_state(&ctx), VALTYPE_I32, LOC_STACK, REG_UNKNOWN, 0);
#else
...
#if __OPT_INIT_STACK__
            push_stack(
              cache_state(&ctx), VALTYPE_I64, LOC_STACK, REG_UNKNOWN, 0);
#else
  ...
  if (!sgxwasm_compile_function_body(&ctx,
                                     type_table,
                                     module_types,
                                     num_funs,
                                     n_frame_locals,
  ...
  patch_stack_frame(
    &ctx, pc_offset_stack_frame, *num_used_spill_slots(&ctx) + 1);
