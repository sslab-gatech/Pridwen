...
int
utimes(const char* filename, const struct timeval tm[2])
{
  printf("sgx_utimes\n");
  return 0;
}
...
int
open(const char* pathname, int flags, unsigned mode)
{
  int retv;
  sgx_status_t sgx_retv;
...
ssize_t
write_opt(int fd, const char* buf, size_t n)
{
  ssize_t retv;
  sgx_status_t sgx_retv;
...
accept(int s, struct sockaddr* addr, socklen_t* addrlen)
{
  int retv;
  sgx_status_t sgx_retv;
  if ((sgx_retv = ocall_sgx_accept(&retv, s, addr, (size_t)*addrlen,
                                   addrlen)) != SGX_SUCCESS) {
...
accept4(int s, struct sockaddr* addr, socklen_t* addrlen, int flags)
{
  int retv;
  sgx_status_t sgx_retv;
  if ((sgx_retv = ocall_sgx_accept4(&retv, s, addr, (size_t)*addrlen, addrlen,
                                    flags)) != SGX_SUCCESS) {
       ...
       const struct sockaddr* dest_addr, size_t addrlen)
{
  ssize_t retv;
  sgx_status_t sgx_retv;
  if ((sgx_retv = ocall_sgx_sendto(&retv, sockfd, buf, len, flags, dest_addr,
                                   addrlen)) != SGX_SUCCESS) {
    printf(" FAILED!, Error code = %d\n", sgx_retv);
    abort();
  }
...
ssize_t
recvfrom(int s, void* buf, size_t len, int flags, struct sockaddr* dest_addr,
         socklen_t* addrlen)
{
  ssize_t retv;
  sgx_status_t sgx_retv;
  if ((sgx_retv = ocall_sgx_recvfrom(&retv, s, buf, len, flags, dest_addr,
                                     *addrlen, addrlen)) != SGX_SUCCESS) {
    printf(" FAILED!, Error code = %d\n", sgx_retv);
    abort();
  }
  return retv;
...
setsockopt(int s, int level, int optname, const void* optval, size_t optlen)
{
  int retv;
  sgx_status_t sgx_retv;
  if ((sgx_retv = ocall_sgx_setsockopt(&retv, s, level, optname, optval,
                                       optlen)) != SGX_SUCCESS) {
    printf(" FAILED!, Error code = %d\n", sgx_retv);
    abort();
  }
...
inet_ntop(int af, const void* src, char* dst, socklen_t size)
{
  ...
  if ((sgx_retv = ocall_sgx_inet_ntop(&inet_ntop_ret, af, src, dst, size)) !=
      SGX_SUCCESS) {
    printf(" FAILED!, Error code = %d\n", sgx_retv);
    abort();
  }
...
int
getdents(unsigned int fd, char* dirp, unsigned int count)
{
  int retv;
  sgx_status_t sgx_retv;
