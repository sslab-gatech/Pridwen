...
ssize_t ocall_sgx_write(int fd, const char *buf, size_t n)
{
    ...
    fprintf(stderr, "write(%d, %s, %u), return: %ld\n", fd, buf, n, ret);
    return ret;
#else
...
ssize_t ocall_sgx_write_opt(int fd, const char *buf, size_t n)
{
...
off_t ocall_sgx_lseek(int files, off_t offset, int whence)
{
#ifdef OCALL_TRACE
    ssize_t ret = lseek(files, offset, whence);
    fprintf(stderr, "lseek(%d, %u, %u), return: %ld\n", files, offset, whence, ret);
    return ret;
#else
    return lseek(files, offset, whence);
#endif
}
...
int ocall_sgx_lstat(const char *pathname, struct stat *statbuf) {
#ifdef OCALL_TRACE
    int ret = lstat(pathname, statbuf);
    fprintf(stderr, "lstat(%s, 0x%p), return: %d\n", pathname, statbuf, ret);
    fprintf(stderr, "st_mode: %07o, uid: %d, gid: %d\n", statbuf->st_mode, statbuf->st_uid, statbuf->st_gid);
    return ret;
#else
...
int ocall_sgx_putc(int c, FILE* f) {
  return putc(c, f);
}

int ocall_sgx_fflush(FILE* s) {
#ifdef OCALL_TRACE
    int ret = fflush(s);
    fprintf(stderr, "fflush(), return: %d\n", ret);
    return ret;
#else
  return fflush(s);
#endif
...
int ocall_sgx_setrlimit(int resource, struct rlimit *rlim) {
#ifdef OCALL_TRACE
    int ret = setrlimit(resource, rlim);
    fprintf(stderr, "setrlimit(%d, 0x%x), return: %d\n", resource, rlim, ret);
    return ret;
#else
    fprintf(stderr, "setrlimit, max: %zu, cur: %zu\n", rlim->rlim_max, rlim->rlim_cur);
...
int ocall_sgx_getdents(unsigned int fd, char *buf, unsigned int count) {
    int ret = syscall(SYS_getdents64, fd, buf, count);
#ifdef OCALL_TRACE
    fprintf(stderr, "getdents(%d), return: %ld\n", fd, ret);
    return ret;
#endif
    return ret;
}
