...
static int fdevent_poll_event_set(fdevents *ev, int fde_ndx, int fd, int events) {
	...
	if (events & FDEVENT_OUT) pevents |= POLLOUT;
        ...
        sgx_log("fdevent_poll_event_set(ev, %d, %d, %d)\n", fde_ndx, fd, events);
	...
	if (ev->unused.used > 0) {
		int k = ev->unused.ptr[--ev->unused.used];
                ...
                sgx_log("fdevent_poll_event_set unused > 0 - k: %d\n", k);
		ev->pollfds[k].fd = fd;
		...
		if (ev->size == 0) {
			...
			force_assert(NULL != ev->pollfds);
                        sgx_log("fdevent_poll_event_set size = 0\n");
		} else if (ev->size == ev->used) {
			ev->size += 16;
			ev->pollfds = realloc(ev->pollfds, sizeof(*ev->pollfds) * ev->size);
			force_assert(NULL != ev->pollfds);
                        sgx_log("fdevent_poll_event_set size = used\n");
		}

                sgx_log("fdevent_poll_event_set used: %d\n", ev->used);
		ev->pollfds[ev->used].fd = fd;
		ev->pollfds[ev->used].events = pevents;
                sgx_log("Set: ev->pollfds[%d].fd = %d, ev->pollfds[%d].events = %d\n", ev->used, fd, ev->used, pevents);
                sgx_log("Check: ev->pollfds[%d].revents = %d\n", ev->used, ev->pollfds[ev->used].revents);
...
static int fdevent_poll_event_next_fdndx(fdevents *ev, int ndx) {
	size_t i;
        ...
        sgx_log("fdevent_poll_event_next_fdndx(ev, %d)\n", ndx);
	i = (ndx < 0) ? 0 : ndx + 1;
	for (; i < ev->used; i++) {
                sgx_log("%d: ev->pollfds[%d].revents: %d\n", i, i, ev->pollfds[i].revents);
		if (ev->pollfds[i].revents) return i;
	}

        sgx_log("fdevent_poll_event_next_fdndx return -1\n");
	return -1;
}
