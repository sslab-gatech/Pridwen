...
static int connection_handle_read_state(server *srv, connection *con)  {
	...
	int is_request_start = chunkqueue_is_empty(cq);
        ...
        sgx_log("connection_handle_read_state\n");
	if (con->is_readable) {
		con->read_idle_ts = srv->cur_ts;
	...
	chunkqueue_remove_finished_chunks(cq);
        ...
        sgx_log("read return > 0, request_count: %d, is_request_start: %d\n", con->request_count, is_request_start);
...
static handler_t connection_handle_fdevent(server *srv, void *context, int revents) {
	connection *con = context;
        ...
        sgx_log("connection_handle_fdevent\n");
...
connection *connection_accept(server *srv, server_socket *srv_socket) {
	...
	if (-1 == cnt) {
                sgx_log("check accept errno (%p): %d\n", &errno, errno);
		switch (errno) {
		case EAGAIN:
...
#else
	//log_error_write(srv, __FILE__, __LINE__, "sdXsd", "read ", con->fd, (unsigned long)mem, " ", mem_len);
...
int connection_state_machine(server *srv, connection *con) {
	int done = 0, r;
        ...
        sgx_log("connection_state_machine - fd: %d, state: %lu, %s\n", con->fd, con->state, connection_get_state(con->state));
		...
		if (r != events) {
			...
			if ((r & FDEVENT_OUT) && !(events & FDEVENT_OUT)) {
				con->write_request_ts = srv->cur_ts;
                        ...
                        sgx_log("connection_state_machine: fdevent_event_set\n");
