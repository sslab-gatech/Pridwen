...
size_t raw_size = 101469; // Hard-coded
int image_height = 149;
int image_width = 227;
...
#endif
#if 0
  FILE *tmp = fopen("testdecomp.dat", "wb");
  fwrite(raw_image, image_height * image_width * 3, 1, tmp);
  fclose(tmp);
#endif
#if COMP == 1
  FILE *tmp = fopen("testdecomp.dat", "rb");
  raw_image = malloc(raw_size);
  fread(raw_image, raw_size, 1, tmp);
#endif
#if COMP == 1
