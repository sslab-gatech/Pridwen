...
// Use to track last inserted/updated branch.
...
insert_jump(struct CompilerContext* ctx, struct CFGNode* node,
            struct CFGTarget* target, size_t type)
{
  ...
  // Special base - the block ends with br_if
  // block:
  //   ...
  //   br_if
  //   ...   <---- jump to here if condition is false.
  // In the case, we need to insert additional jump.
               ...
               control_type_t control_type, size_t depth)
{
  ...
  if (extra_bytes != 0) {
    last_branch = control_type;
  }

#if __DEBUG_CASLR__
  plog("[onControlStart] (%zu, %zu)\n", node->id, node->size);
#endif
