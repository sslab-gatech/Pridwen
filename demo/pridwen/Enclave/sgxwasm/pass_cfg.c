...
static int
is_ending_with_jmp(struct CompilerContext* ctx, size_t size)
{
               ...
               size_t depth)
{
  ...
  size_t low_instr_offset = num_low_instrs(ctx);
  size_t low_instr_diff = num_low_instrs(ctx) - node->low_instr_offset;
  size_t new_id = node->id + 1;
  size_t skip = is_ending_with_jmp(ctx, diff);

  (void)func;

  // Workaround: Do not skip at the beginning of the control_block.
  if (skip == 1 && last_branch != OPCODE_BR_IF &&
      control_type != CONTROL_BLOCK) {
  ...
  node->size = diff;
  node->low_instr_num = low_instr_diff;
#if __DEBUG_CFG__
  plog("[onControlStart] (node, size, low_instr_num) - (%zu, %zu, %zu)\n",
       node->id, node->size, node->low_instr_num);
#endif
  ...
  node->id = new_id;
  node->offset = offset;
  node->low_instr_offset = low_instr_offset;
  node->depth = depth;
             ...
             size_t depth)
{
  ...
  if (skip == 1 && last_branch != OPCODE_BR_IF) {
    ...
    target->depth = 0;
    target->control_type = control_type;
    target->control_state = ControlEnd;
  }
  node->size = diff;
  node->low_instr_num = low_instr_diff;
#if __DEBUG_CFG__
  plog("[onControlEnd] (node, size, low_instr_num) - (%zu, %zu, %zu)\n",
       node->id, node->size, node->low_instr_num);
#endif
                 ...
                 const struct Instr* instr)
{
  ...
  size_t diff = offset - node->offset;
  size_t low_instr_offset = num_low_instrs(ctx);
  size_t low_instr_diff = low_instr_offset - node->low_instr_offset;
...
#endif
      target->id = node->id + 1;
      target->control_type = c->type;
      target->control_state = BranchEnd;
...
#endif
      target->id = 0;
      target->control_type = CONTROL_IF;
      target->control_state = BranchEnd;
...
#endif
      target->id = 0;
      target->control_type = CONTROL_ELSE;
      target->control_state = BranchEnd;
      target->type = ElseUnknown;
      target->offset = pc_offset(output(ctx)) - 5;
      target->depth = depth;
