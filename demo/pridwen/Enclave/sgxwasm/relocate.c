...
void
set_code_entry_offset(struct CodeUnitTable* table, size_t fun_index,
                      uint64_t addr)
{
...
#if DEBUG_RELOCATE
#if __linux__
  printf("[set_code_entry_offset] fun_index: %zu, addr: 0x%lx\n", fun_index,
         addr);
#else
  printf("[set_code_entry_offset] fun_index: %zu, addr: 0x%llx\n", fun_index,
         addr);
#endif
#endif
}
...
void
add_relo_entry(struct RelocationTable* table, size_t fun_index,
               relocation_type_t type, struct MemoryRef* memref)
{
  ...
  if (type == RELO_CODE_UNIT) {
    printf("[add_relo_entry] fun: %zu, unit %zu, offset: %lld, size: %zu\n",
           fun_index, entry->unit_index, entry->offset, entry->unit_size);
  }
  if (type == RELO_JMP_NEXT) {
    printf("[add_relo_entry] fun: %zu, unit: %zu -> %zu (%zu), offset: %lld\n",
           fun_index, entry->unit_index, entry->target_index,
           memref->target_type - 0xe0, entry->offset);
  }
#endif
#if DEBUG_RELOCATE
  printf("[add_relo_entry] fun_index: %zu, offset: %lld, target_index: %zu\n",
         fun_index, entry->offset, entry->target_index);
#endif
}

void
relocate(struct Module* module, struct RelocationTable* table,
         struct IndirectCallTable* indirect_table,
         struct CodeUnitTable* code_table, struct Springboard* springboard,
         uint64_t ssa_polling_addr, int use_code_unit)
{
  size_t i = 0;
  uint64_t code_base = sgxwasm_get_code_base();
...
uint64_t counter = 0;
unsigned mem[4096];
unsigned volatile sync_state = 0;
#endif
void
sgxwasm_counting()
{
...
#if __DEBUG_COLOC_TEST__
void
debug_exit_type(uint32_t exittype)
{
  printf("[debug_exit_type] %u\n", exittype);
}
#endif
#endif
...
// T-SGX Support.
void
construct_springboard(struct Springboard* springboard, uint64_t ssa_addr)
{
  void* mapped;
...
#if __COLOC_TEST__
  struct Operand ssa;
  emit_movq_ri(&output, ScratchGP2, ssa_addr);
  build_operand(&ssa, ScratchGP2, REG_UNKNOWN, SCALE_NONE, 0);
  emit_movb_rm(&output, ScratchGP, &ssa);
  emit_cmpb_ri(&output, ScratchGP, VARYS_MAGIC);
  emit_jcc(&output, COND_EQ, &begin, Near);
  SaveRegisterStates(&output);
#if !__DEBUG_COLOC_TEST__
  emit_movq_ri(&output, GP_RAX, (uint64_t)colocation_test);
#else
  emit_movq_ri(&output, GP_RAX, (uint64_t)debug_exit_type);
#endif
  //emit_mov_rr(&output, GP_RDI, ScratchGP, VALTYPE_I32);
  emit_call_r(&output, GP_RAX);
  RestoreRegisterStates(&output);
  emit_mov_mi(&output, &ssa, VARYS_MAGIC, VALTYPE_I32);
#endif
...
void
add_code_unit(struct CodeUnitTable* table, size_t fun_index,
              struct MemoryRef* memref)
{
  ...
  printf("[add_code_unit] fun: %zu, unit %zu, offset: %llu, size: %zu\n",
         fun_index, num_units - 1, unit->offset, unit->size);
#endif
}

void
convert_to_unit_relo_info(size_t fun_index, struct RelocationTable* relo_table,
    ...
    for (j = unit_list->size - 1; j != 0; j--) {
      ...
      if (entry->offset >= unit->offset) {
        ...
        printf("[convert_unit_relo_into] fun %zu offset: %llx -> %llx, id: %zu "
               "-> %zu\n",
               fun_index, entry->offset, entry->offset - unit->offset,
               entry->unit_index, j);
#endif
...
// ASLR support.
void*
randomize_layout(struct Function* func, struct CodeUnitTable* table, void* code,
                 size_t code_size, int rand)
{
  assert(func != NULL);
...
void
construct_ssa_polling(uint64_t ssa_addr, uint64_t* fun_addr)
{
  void* mapped;
  struct SizedBuffer output = { 0, 0, NULL };
  struct Operand ssa;
  ...
  emit_movb_rm(&output, GP_R14, &ssa);
  ...
  emit_cmpb_ri(&output, GP_R14, VARYS_MAGIC);
