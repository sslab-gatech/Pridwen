...
rand()
{
  ...
  if ((sgx_retv =
         sgx_read_rand((unsigned char*)&retv, sizeof(retv)) != SGX_SUCCESS)) {
...
strftime(char* s, size_t max, const char* format, const struct tm* tm)
{
  size_t retv;
  sgx_status_t sgx_retv;
  if ((sgx_retv = ocall_sgx_strftime(&retv, s, max, format, tm)) !=
      SGX_SUCCESS) {
    printf(" FAILED!, Error code = %d\n", sgx_retv);
    abort();
  }
...
open(const char* pathname, int flags, ...)
{
  ...
  sgx_status_t sgx_retv;
  if ((sgx_retv = ocall_sgx_open(&retv, pathname, flags, mode)) !=
      SGX_SUCCESS) {
    printf(" FAILED!, Error code = %d\n", sgx_retv);
    abort();
  }
...
ssize_t
read(int fd, char* buf, size_t buf_len)
{
  ssize_t retv;
  sgx_status_t sgx_retv;
...
ssize_t
write(int fd, const char* buf, size_t n)
{
  ssize_t retv;
  sgx_status_t sgx_retv;
...
lseek(int fildes, off_t offset, int whence)
{
  off_t retv;
  sgx_status_t sgx_retv;
  if ((sgx_retv = ocall_sgx_lseek(&retv, fildes, offset, whence)) !=
      SGX_SUCCESS) {
    printf(" FAILED!, Error code = %d\n", sgx_retv);
    abort();
  }
...
int
bind(int s, const struct sockaddr* addr, size_t addr_size)
{
  int retv;
  sgx_status_t sgx_retv;
...
int
connect(int s, const struct sockaddr* addr, size_t addrlen)
{
  int retv;
  sgx_status_t sgx_retv;
...
accept(int s, struct sockaddr* addr, socklen_t* addrlen)
{
  int retv;
  sgx_status_t sgx_retv;
  if ((sgx_retv = ocall_sgx_accept(
         &retv, s, addr, (size_t)*addrlen, addrlen)) != SGX_SUCCESS) {
    printf(" FAILED!, Error code = %d\n", sgx_retv);
    abort();
  }
...
accept4(int s, struct sockaddr* addr, socklen_t* addrlen, int flags)
{
  int retv;
  sgx_status_t sgx_retv;
  if ((sgx_retv = ocall_sgx_accept4(
         &retv, s, addr, (size_t)*addrlen, addrlen, flags)) != SGX_SUCCESS) {
    printf(" FAILED!, Error code = %d\n", sgx_retv);
    abort();
  }
       ...
       size_t addrlen)
{
  ssize_t retv;
  sgx_status_t sgx_retv;
  if ((sgx_retv = ocall_sgx_sendto(
         &retv, sockfd, buf, len, flags, dest_addr, addrlen)) != SGX_SUCCESS) {
    printf(" FAILED!, Error code = %d\n", sgx_retv);
    abort();
  }
  return retv;
         ...
         socklen_t* addrlen)
{
  ssize_t retv;
  sgx_status_t sgx_retv;
  if ((sgx_retv = ocall_sgx_recvfrom(
         &retv, s, buf, len, flags, dest_addr, *addrlen, addrlen)) !=
      SGX_SUCCESS) {
    printf(" FAILED!, Error code = %d\n", sgx_retv);
    abort();
  }
...
sendfile(int out_fd, int in_fd, off_t* offset, size_t count)
{
  ssize_t retv;
  sgx_status_t sgx_retv;
  if ((sgx_retv = ocall_sgx_sendfile(&retv, out_fd, in_fd, offset, count)) !=
      SGX_SUCCESS) {
    printf(" FAILED!, Error code = %d\n", sgx_retv);
    abort();
  }
            ...
            unsigned long* res)
{
  ...
  int retv, i;
  sgx_status_t sgx_retv;
  if ((sgx_retv = ocall_sgx_getaddrinfo(&retv, node, service, hints, res)) !=
      SGX_SUCCESS) {
    printf(" FAILED!, Error code = %d\n", sgx_retv);
    abort();
  }
...
int
getsockname(int s, struct sockaddr* name, socklen_t* addrlen)
{
  int retv;
  sgx_status_t sgx_retv;
  if ((sgx_retv = ocall_sgx_getsockname(
         &retv, s, name, (size_t)*addrlen, addrlen)) != SGX_SUCCESS) {
    printf(" FAILED!, Error code = %d\n", sgx_retv);
    abort();
  }
  return retv;
...
getsockopt(int s, int level, int optname, void* optval, socklen_t* optlen)
{
  int retv;
  sgx_status_t sgx_retv;
  if ((sgx_retv = ocall_sgx_getsockopt(
         &retv, s, level, optname, optval, *optlen, optlen)) != SGX_SUCCESS) {
    printf(" FAILED!, Error code = %d\n", sgx_retv);
    abort();
  }
  return retv;
...
int
setsockopt(int s, int level, int optname, const void* optval, size_t optlen)
{
  int retv;
  sgx_status_t sgx_retv;
  if ((sgx_retv = ocall_sgx_setsockopt(
         &retv, s, level, optname, optval, optlen)) != SGX_SUCCESS) {
    printf(" FAILED!, Error code = %d\n", sgx_retv);
    abort();
  }
...
sighandler_t
signal(int signum, sighandler_t a)
{
  sighandler_t retv;
  sgx_status_t sgx_retv;
...
poll(struct pollfd* fds, nfds_t nfds, int timeout)
{
  ...
  if ((sgx_retv = ocall_sgx_poll(&retv, fds, fds_size, nfds, timeout)) !=
      SGX_SUCCESS) {
    printf(" FAILED!, Error code = %d\n", sgx_retv);
    abort();
  }
...
int
select(int nfds, fd_set* reafds, fd_set* writefds, fd_set* exceptfds, struct timeval* timeout)
{
  return 1;
}
...
fseek(FILE* file, long offset, int origin)
{
  int retv;
  sgx_status_t sgx_retv;
  if ((sgx_retv = ocall_sgx_fseek(&retv, file, offset, origin)) !=
      SGX_SUCCESS) {
    printf(" FAILED!, Error code = %d\n", sgx_retv);
    abort();
  }
...
fwrite(const void* buffer, size_t size, size_t count, FILE* stream)
{
  size_t retv;
  sgx_status_t sgx_retv;
  if ((sgx_retv = ocall_sgx_fwrite(&retv, buffer, size, count, stream)) !=
      SGX_SUCCESS) {
    printf(" FAILED!, Error code = %d\n", sgx_retv);
    abort();
  }
...
fread(void* buffer, size_t size, size_t count, FILE* stream)
{
  size_t retv;
  sgx_status_t sgx_retv;
  if ((sgx_retv = ocall_sgx_fread(&retv, buffer, size, count, stream)) !=
      SGX_SUCCESS) {
    printf(" FAILED!, Error code = %d\n", sgx_retv);
    abort();
  }
...
lseek64(int fildes, off_t offset, int whence)
{
  off_t retv;
  sgx_status_t sgx_retv;
  if ((sgx_retv = ocall_sgx_lseek64(&retv, fildes, offset, whence)) !=
      SGX_SUCCESS) {
    printf(" FAILED!, Error code = %d\n", sgx_retv);
    abort();
  }
...
int
open64(const char* pathname, int flags, unsigned mode)
{
  int retv;
  sgx_status_t sgx_retv;
  if ((sgx_retv = ocall_sgx_open64(&retv, pathname, flags, mode)) !=
      SGX_SUCCESS) {
    printf(" FAILED!, Error code = %d\n", sgx_retv);
    abort();
  }
...
{
  printf("sgx_ftruncate: fd: %d, len: %zu\n", fd, length);
  return 0;
}
...
int
mkdir(int dirfd, const char* pathname, mode_t mode)
{
  printf("sgx_mkdir\n");
  return 0;
}
...
ioctl(int fd, unsigned long request, ...)
{
  ...
  sgx_status_t sgx_retv;
  if ((sgx_retv = ocall_sgx_ioctl(&retv, fd, request, arg, size)) !=
      SGX_SUCCESS) {
    printf(" FAILED!, Error code = %d\n", sgx_retv);
    abort();
  }
...
inet_ntop(int af, const void* src, char* dst, socklen_t size)
{
  ...
  retv = (char*)malloc(size);
  if ((sgx_retv = ocall_sgx_inet_ntop(&retv, af, src, dst, size)) !=
      SGX_SUCCESS) {
    printf(" FAILED!, Error code = %d\n", sgx_retv);
    abort();
  }
...
int
inet_pton(int ad, const char* src, void* dst)
{
  int retv;
  sgx_status_t sgx_retv;
...
clock_gettime(clockid_t clk_id, struct timespec *tp)
{
  int retv;
  sgx_status_t sgx_retv;
  if ((sgx_retv = ocall_sgx_clock_gettime(&retv, clk_id, tp)) != SGX_SUCCESS) {
    printf(" FAILED!, Error code = %d\n", sgx_retv);
    abort();
  }
  return retv;
...
readv(int fd, const struct iovec* iov, int iovcnt)
{
  ...
  for (i = 0; i < iovcnt; i++) {
    ...
    if ((sgx_retv = ocall_sgx_read(&n, fd, buf, len)) != SGX_SUCCESS) {
      printf(" FAILED!, Error code = %d\n", sgx_retv);
      abort();
    }
    retv += n;
